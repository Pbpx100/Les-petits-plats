...
inputPrincipal.addEventListener('input', () => {
    ...
    if (mainInputValue.length > 3) {
        ...
        let allAppliance = '';
        cleanFilterAndPage();
        mappingRecipes(recipes).forEach((recipe) => {
        ...
        if (arrayRecipes.length === 0) {
            totalRecettes.innerHTML = `${arrayRecipes.length} Recettes`
            artRecipes.innerHTML = 'Result not found';
        }
        // totalRecettes.innerHTML = `${arrayRecipes.length} Recettes`
        if (arrayRecipes.length < 10) {
            totalRecettes.innerHTML = `0${arrayRecipes.length} Recettes`
